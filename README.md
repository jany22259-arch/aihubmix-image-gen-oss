...
| 多模型切换 | `qwen-image-2.0`（中文/电商）/ `gpt-image-2`（最高画质）/ `gemini-3-pro-image`（多模态路由）/ `wan2.7-image-pro` / `doubao-seedream-5.0-lite` |
...
| `wan2.7-image-pro` | 走 `/v1/images/generations`，适合 Wan/万相/通义系风格生图 |
| `doubao-seedream-5.0-lite` | 走 `/v1/images/generations`，豆包 Seedream 轻量模型，适合快速/低成本出图 |
...
| `--output-dir` | 是 | — | 图片保存目录 |
| `--model` | 否 | `qwen-image-2.0` | 生图模型：`qwen-image-2.0`, `gpt-image-2`, `gemini-3-pro-image`, `wan2.7-image-pro`, `doubao-seedream-5.0-lite` |
| `--size` | 否 | `1024x1024` | 图片尺寸 |
| `--n` | 否 | `1` | `/images/generations` 路由模型的生成数量；Gemini 当前仅支持 1 |
...
| `Unsupported model: xxx` | 模型名写错 | 用 `qwen-image-2.0` / `gpt-image-2` / `gemini-3-pro-image` / `wan2.7-image-pro` / `doubao-seedream-5.0-lite` |
